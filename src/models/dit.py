...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.input_size = input_sz[1]
        self.patch_size = patch_size
        self.in_channels = input_sz[0]
        ...
        self.d_embedder = TimestepEmbedder(hidden_size, t_scale=t_scale) if shortcut else None
    ...
    def initialize_weights(self):
        ...
        # Initialize shortcut timestep embedding MLP if it exists:
    ...
    def forward(self, x, t, y=None, return_tokens=False, **kwargs):
        ...
        if self.d_embedder is not None:
            assert 'd' in kwargs, "Shortcut timestep 'd' must be provided in kwargs when d_embedder is used."
            d = self.d_embedder(kwargs.get('d', torch.zeros_like(t)))  # (B, D)
            cond = cond + d  # incorporate shortcut timestep embedding into the conditioning
