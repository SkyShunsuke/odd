...
def calculate_px_metrics(
    ...
    if 'px_f1max' in metrics:
        # keep the flat tensors on CPU: f1_max_gpu_hist streams them to the
        # GPU in chunks, so huge pixel sets no longer OOM the device
        gt_masks_flat = (gt_masks_flat > 0).astype(np.float32)
        scores_tensor = torch.from_numpy(np.ascontiguousarray(pred_scores_flat)).to(torch.float32)
        labels_tensor = torch.from_numpy(gt_masks_flat)
        f1, _ = f1_max_gpu_hist(scores_tensor, labels_tensor, distributed=distributed, device=device)
